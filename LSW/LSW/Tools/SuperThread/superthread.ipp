...
namespace LSW {
	namespace v5 {
		namespace Tools {
			...
			inline void SuperThreadT<T>::_set_promise_forced()
			{
				promise.set_value(T{});
			}
			...
			template<typename T>
			inline SuperThreadT<T>::SuperThreadT(const superthread::performance_mode& m)
			{
				Handling::init_basic();
			...
			template<typename T>
			inline SuperThreadT<T>::SuperThreadT(const std::function<T(boolThreadF)> f)
			{
				Handling::init_basic();
				set(f);
			}
			...
			template<typename T>
			template<typename Q, std::enable_if_t<!std::is_void_v<Q>, int>>
			inline void SuperThreadT<T>::set(const std::function<T(boolThreadF)> f)
			{
				join();
			...
			template<typename T>
			inline Future<T> SuperThreadT<T>::start()
			{
				join();
			...
			template<typename T>
			inline void SuperThreadT<T>::stop()
			{
				_die_already = true;
			...
			template<typename T>
			inline void SuperThreadT<T>::join()
			{
				if (thr) {
					stop();
			...
			template<typename T>
			inline void SuperThreadT<T>::kill()
			{
				if (thr) {
					stop();
			...
			template<typename T>
			inline bool SuperThreadT<T>::running() const
			{
