...
namespace LSW {
	namespace v5 {
		namespace Interface {
			...
			template<> 
			inline size_t Config::get_as(const std::string& sec, const std::string& key) const
			{
				auto res = get(sec, key);
				size_t varr;
			...
			template<>
			inline std::vector<size_t> Config::get_array(const std::string& sec, const std::string& key)  const
			{
				if (!_has(sec)) throw Handling::Abort(__FUNCSIG__, "There is no section called " + sec + " in this config file (" + path + ")");
