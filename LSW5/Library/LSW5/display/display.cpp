	...
	namespace v5 {
		...
		void Display::__checkacknowledge()
		{
			if (should_acknowledge && std::chrono::system_clock::now().time_since_epoch() - time_last_acknowledge > display::acknowledge_timeout_time) {
				...
				display_itself_follower->custom(display_itself);
				display_itself_follower->set_as_reference_target();
				...
				if (use_doublebuffer) {
					...
					newbmp->set_as_target();

					if (display_buffer) {
						display_buffer->draw(0, 0, newbmp->get_width(), newbmp->get_height()); // later on it sets itself as newbmp, no worry
						__print("Double buffering has updated and is now " + std::string((newbmp && *newbmp) ? "ENABLED SUCCESSFULLY" : "NOT ENABLED, FAILED"));
					}
					else {
					...
					bmps.remove("__display_b_" + display_buffer_name);
		...
		bool Display::launch(const size_t option)
		{
			...
			if (display_itself = al_create_display(display_now_config.chosen_mode.width, display_now_config.chosen_mode.height)) {
				...
				SuperResource<Bitmap> bmps;
				display_itself_follower = bmps.create("__display_itself_" + display_buffer_name);
				display_itself_follower->custom(display_itself);
				display_itself_follower->set_as_reference_target();
				display_itself_follower->set_as_target();
			}
			else {
		...
		void Display::close()
		{
			...
			if (display_buffer) {
				SuperResource<Bitmap> bmps;
				bmps.remove(display_buffer_name);
				display_buffer.reset();
			}
			...
			if (display_itself) {
				SuperResource<Bitmap> bmps;
				bmps.remove("__display_itself_" + display_buffer_name);
				display_itself_follower.reset();
		...
		void Display::flip()
		{
			if (display_buffer) {
				...
				if (ref_fx_amount == 0.0) {
					...
					display_buffer->draw(0, 0, al_get_display_width(display_itself), al_get_display_height(display_itself), 0);
				}
				else {
					double conv_quick_fx = pow(ref_fx_amount, 2.5);
					double conv_quick_fx2 = pow(ref_fx_amount, 0.8);
					double offsets[3];
					double scl = 10.0 * sqrt(display_buffer->get_width() * display_buffer->get_height()) / sqrt(display::display_reference_size[0] * display::display_reference_size[1]);
					...
					display_buffer->draw(al_map_rgba_f(qfbl, 0.000, 0.000, qfbl),  offsets[0], offsets[1], al_get_display_width(display_itself) + offsets[1], al_get_display_height(display_itself) + offsets[2]);	
					display_buffer->draw(al_map_rgba_f(0.000, qfbl, 0.000, qfbl),  offsets[1], offsets[2], al_get_display_width(display_itself) + offsets[2], al_get_display_height(display_itself) + offsets[0], 0);
					display_buffer->draw(al_map_rgba_f(0.000, 0.000, qfbl, qfbl),  offsets[2], offsets[0], al_get_display_width(display_itself) + offsets[0], al_get_display_height(display_itself) + offsets[1], 0);
					display_buffer->draw(al_map_rgba_f(0.015, 0.015, 0.015, 0.015), 0, 0, al_get_display_width(display_itself), al_get_display_height(display_itself), 0);
			...
			if (display_buffer && *display_buffer) {
				display_buffer->set_as_reference_target();
				display_buffer->set_as_target();
			}
			else {
				//al_set_target_backbuffer(display_itself);
				display_itself_follower->set_as_reference_target();
